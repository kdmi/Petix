...
- Battle energy (`api/_lib/battle-energy.js`) is stored as `energyUsed` (spent today, resets at New York midnight) plus, since feature 020, `energyPurchased` (bought fights, never expire, spent after free ones) and `energyPacks` (last purchase per pack index → 24h cooldown). `energyCurrent`/`energyMax` in views include purchased energy. Packs/cooldown/switch are `ENERGY_*` keys in `economy-config`; purchase goes through `POST /api/character/buy-energy` (`api/_lib/energy-shop.js`). See [specs/020-energy-shop/](specs/020-energy-shop/).
...
- [specs/019-petix-token-evm/quickstart.md](specs/019-petix-token-evm/quickstart.md) — роли кошельков, тестовая монета на Pons, preflight, тихий тест на проде, день запуска, инциденты
- [specs/019-petix-token-evm/tasks.md](specs/019-petix-token-evm/tasks.md) — 47 задач, TDD по фазам, 5 операционных
- [specs/017-boss-campaign/spec.md](specs/017-boss-campaign/spec.md) — feature spec: «Путь к короне», PvE-кампания «три в ряд» из 50 боссов; изолированная демо в `match-demo/` (без кошелька и реальных Points), тесты `tests/unit/pet-campaign.test.js`
- [specs/017-boss-campaign/implementation-status.md](specs/017-boss-campaign/implementation-status.md) — что реально сделано в демо и что нужно для интеграции с реальным Petix (раздел 7)
- [specs/017-boss-campaign/balance-and-bosses.md](specs/017-boss-campaign/balance-and-bosses.md) — формулы урона/заряда, каталог 50 боссов, бюджет наград
- [specs/017-boss-campaign/simulation-results.md](specs/017-boss-campaign/simulation-results.md) — симуляция достижимости (`node match-demo/simulate.js`): порог падения побед по составам, входы против наград, выводы для баланса
